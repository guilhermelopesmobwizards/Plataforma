...
function buildBarData(mode) {
    ...
    const cfgs = {
        ...
        cost: { key: "cost", color: "rgba(96,165,250,", label: "Custo EUR" },
        margin: { key: "margin", color: "rgba(74,222,128,", label: "Margem EUR" },
    };
...
function renderTable(results, count, page, pages) {
    ...
    if (!results.length) {
        tbody.innerHTML = `<tr><td colspan="12" style="text-align:center;padding:40px;color:var(--muted);">
            Sem dados. Importa um ficheiro Excel para começar.
        </td></tr>`;
        document.getElementById("table-info").textContent = "0 resultados";
        document.getElementById("table-sub").textContent = "0 campanhas";
    ...
    tbody.innerHTML = results.map(c => {
        ...
        const margin = (parseFloat(m.revenue_eur) || 0) - (parseFloat(m.cost_eur) || 0);
        ...
        return `<tr>
          ...
          <td class="num">${fmtEur(m.cost_eur)}</td>
          <td class="num" style="color:var(--accent2);">${fmtEur(m.revenue_eur)}</td>
          <td class="num" style="color:${margin >= 0 ? "var(--accent2)" : "var(--danger)"};">${fmtEur(margin)}</td>
          <td class="num">${m.cpa != null ? parseFloat(m.cpa).toFixed(2) : "—"}</td>
          <td class="num">${m.conv != null ? fmtNum(m.conv) : "—"}</td>
          <td class="num" style="color:${roi >= 0.3 ? "var(--accent2)" : roi >= 0.1 ? "var(--warning)" : "var(--danger)"};">
              ${m.roi != null ? fmtRoi(m.roi) : "—"}</td>
          <td style="font-size:11px;">${statusBadge(cp.ok_conv, cp.ok_rev)}</td>
        </tr>`;
    }).join("");

    document.getElementById("table-info").textContent =
        `A mostrar ${(page - 1) * 25 + 1}–${Math.min(page * 25, count)} de ${count}`;
    document.getElementById("table-sub").textContent = `${count} campanha${count !== 1 ? "s" : ""}`;
...
function exportCSV() {
    const header = ["ID", "Cliente", "País", "Plataforma", "Categoria", "Custo EUR", "Receita EUR", "Margem EUR", "CPA", "Conv", "ROI"];
    const rows = [[...header]];
